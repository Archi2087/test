...
const FAVORITES_KEY = 'favorites';   // *** ИЗБРАННОЕ
...
window.onload = () => {
    ...
    updateFavoritesButton();   // *** ИЗБРАННОЕ
};
...
function deleteTestFromLibrary(name) {
    ...
    delete lib[name];
    saveLibrary(lib);
    // очистка статистики ошибок и избранного для удаляемого теста
    clearMistakeStatsForTest(name);
    clearFavoritesForTest(name);                // *** ИЗБРАННОЕ
...
function show() {
    ...
    const currentTestName = localStorage.getItem(CURRENT_TEST_KEY);  // *** ИЗБРАННОЕ
    app.innerHTML =
...
function finish() {
    ...
    // сохранение ошибок в накопительную статистику
...
function favoritesMode() {
    ...
    index = 0;
    mistakes = [];
    // Останавливаем таймер, как в mistakesMode
    clearInterval(timerInt);
    timerInt = null;
    time = 0;
    timer.textContent = '';
...
// ══════ конец блока избранного ══════

// ══════ БЛОК: статистика ошибок ══════
