...
const MISTAKES_LIST_KEY = 'mistakesList';   // список ошибок для режима «Ошибки»
...
window.onload = () => {
    ...
    if (saved) {
        ...
        if (currentName && getLibrary()[currentName]) {
            ...
            loadMistakesFromStorage();   // подгружаем ошибки для выбранного теста
        }
    }
...
let time = 0;
let timerInt = null;
let mistakesReviewMode = false;   // true, когда работаем в режиме «Ошибки»
...
function deleteTestFromLibrary(name) {
    ...
    clearMistakesListForTest(name);     // удаляем и список ошибок
...
function selectTest(name, showAlert = true) {
    ...
    loadMistakesFromStorage();   // загружаем ошибки для выбранного теста
    renderTestList();
    updateFavoritesButton();
}
...
function start() {
    ...
    index = 0;
    drawChart();
    // mistakes больше не очищаем — они загружены ранее через loadMistakesFromStorage()
    // и остаются актуальными до тех пор, пока пользователь не удалит их правильными ответами
...
function ans(i) {
    ...
    if (i !== q.correct) {
        ...
        // 1. Накопительная статистика (количество ошибок на вопросе)
        if (currentTestName) {
            addMistake(currentTestName, q.number);
        }

        // 2. Добавляем вопрос в список для режима «Ошибки» (без дубликатов)
        if (currentTestName) {
        ...
        // Правильный ответ → если мы в режиме «Ошибки», удаляем вопрос из этого списка
...
function finish() {
    ...
    saveResult(percent);
    drawChart();
    updateScoreAndAchievements(correct, list.length, percent); // новая строк
    mistakesReviewMode = false;   // выходим из режима ошибок, если были в нём
    clearSession();
...
function restoreSession(session) {
    ...
    time = session.time;
    mistakesReviewMode = false;   // при восстановлении всегда выключаем режим ошибок
...
function updateScoreAndAchievements(correct, total, percent) {
    ...
    if (percent === 100) points += 50; // бонус за безупречный тест
    points += 10; // за завершение любого теста
    totalScore += points;
    localStorage.setItem('totalScore', totalScore);

    // Уровень: каждые 200 очков новый уровень
    playerLevel = Math.floor(totalScore / 200) + 1;
...
function removeOption(qIdx, oIdx) {
    ...
    rows[oIdx].remove();
    // обновить индексы в select правильного ответа
...
let allQuestionsForEdit = []; // временный массив для редактора
...
function saveEditedTest() {
    ...
    // Обновляем текущий all, если этот тест выбран
    if (localStorage.getItem(CURRENT_TEST_KEY) === currentName) {
        all = newQuestions;
    }

    // Обновляем связанные данные: статистика ошибок и избранное могут содержать удалённые номера — оставим как есть
    alert('Тест сохранён!');
    reviewAll(); // возвращаемся в просмотр
} 